...
@router.get("")
def list_products(destacado: Optional[bool] = Query(None), db=Depends(get_db)):
...
def create_product(data: ProductCreate, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
...
def update_product(product_id: str, data: ProductUpdate, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    role = current_user.get("rol", "")
    if role not in ("administrador", "colaborador"):
        raise HTTPException(status_code=403, detail="No tienes permisos")
...
def delete_product(product_id: str, current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    role = current_user.get("rol", "")
