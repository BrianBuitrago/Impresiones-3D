...
class ReportItem(BaseModel):
    quote_id: str = Field("", description="ID de cotización relacionada")
    ...
    clienteNombre: Optional[str] = Field(None, description="Nombre del cliente")
    clienteTelefono: Optional[str] = Field(None, description="Teléfono del cliente")
    origen: Optional[str] = Field("manual", description="web o manual")
...
class ReportBase(BaseModel):
    colaboradorUid: str = Field(..., description="UID del colaborador")
    colaboradorNombre: str = Field(..., min_length=2, max_length=120, description="Nombre del colaborador")
    periodo: str = Field(..., min_length=4, max_length=20, description="Periodo ej. Junio/26")
    categorias: List[str] = Field(default_factory=list, description="Categorías del colaborador")
    items: List[ReportItem] = Field(..., min_items=1, description="Items del reporte")
    notas: Optional[str] = Field("", max_length=1000, description="Notas generales")
    ...
    def validate_periodo(cls, value):
        ...
        if '/' not in normalized:
            raise ValueError('El periodo debe usar formato MM/AA, ej: Junio/26')
        return normalized
...
class ReportUpdate(BaseModel):
    items: Optional[List[ReportItem]] = None
